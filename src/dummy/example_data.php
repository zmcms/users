...
foreach($backend_user as $a){
	DB::table($tblName)->insert([ 'token' => $a['token'], 'mail' => $a['mail'], 'login' => $a['login'], 'active' => $a['active'], 'role_id' => $a['role_id'], 'name' => $a['name'], 'last_name' => $a['last_name'], 'nick' => $a['nick'], 'sex' => $a['sex'], 'ilustration' => $a['ilustration'], 'birthday' => $a['birthday'], 'confirmed' => $a['confirmed'], 'login_try' => $a['login_try'], 'total_login_try' => $a['total_login_try'], 'homepage' => $a['homepage'],]);
};
